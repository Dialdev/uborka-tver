...
class acf_field_post_object_service extends acf_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	{
		// vars
		$this->name = 'post_object_service';
		$this->label = 'Страница услуги';
		...
		foreach( $field['post_type'] as $post_type )
		{
			...
			if($posts) {
				...
				foreach( $posts as $p ) {
                                        ...
                                        if(
                                                get_page_template_slug($p->ID) != 'template_service.php' && 
                                                get_page_template_slug($p->ID) != 'template_services.php' && 
                                                get_page_template_slug($p->ID) != 'template_robot.php' && 
                                                get_page_template_slug($p->ID) != 'template_tork.php'&& 
                                                get_page_template_slug($p->ID) != 'template_salfetki.php'&& 
                                                get_page_template_slug($p->ID) != 'template_listing.php'
                                        ) continue;
...
<tr style="display:none" class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label for=""><?php _e("Post Type Service",'acf'); ?></label>
	</td>
	<td>
		<?php 
	...
	function format_value_for_api( $value, $post_id, $field )
	{
		...
		if( is_array($value) )
		{
			...
			$posts = get_posts(array(
				'numberposts' => -1,
				'post__in' => $value,
				'post_type'	=> 'page',//	apply_filters('acf/get_post_types', array()),
				'post_status' => array('publish', 'private', 'draft', 'inherit', 'future'),
			));
...
new acf_field_post_object_service();
